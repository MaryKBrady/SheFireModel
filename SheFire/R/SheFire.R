...
shefire <- function(input, sensor.depths = c(5,10, 15), cutOff = 1440, override.clip = F, moving.window = F, window.size = 3, 
  ...
  if(print.plots.tables != T & save.plots.tables != F){
  ...
  if(moving.window == F & override.clip == F) {
    ...
    if(StartInd <= 0){  #if time.buffer sets StartInd to be negative
      ...
      print("Warning: StartInd was set to the first time point because the time.buffer was set too large for this input data (it would have set the StartInd prior to the start of the data)")
    }
  } 
  #Setting EndInd
  if(override.clip == F){
  ...
  #printing and saving
  if(print.plots.tables == T & regression == F){  #print all plots and tables at the end if continuing to regressions
    print(input_plot)
    print(Figure_an)
    print(All3)
    print(summary.table.df)
  }
  if(save.plots.tables){
  ...
  if (Shallowest <= 0){ #if shallowest not ok
    ...
    print(paste("The shallowest depth the model can mathematically extrapolate to is", Shallowest, "cm. If you need a shallower depth, try increasing the time.buffer. The model can become unreasonable at particularly shallow depths in some fires so the shalowest reasonable extrapolation may be a little deeper than that (plot shallowest depth temperature over time along side a deeper depth like 1 or 2 cm to visually assess reasonability if you need specific data at or near the shallowest depth)"))
  } else {#if shallowest ok but pearson not ok
    stop(paste("Calculating regressions failed because the pearson correlation for Shape parameter was lower than the threshold with a value of:", Shape.pearson, ". Decreasing the time.buffer should improve the fit. Or adjust the pearson correlation threshold parameter"))
  }
  ...
  model.list <- list("BFDEquation" = BFDEquation, "MaxTemp.reg" = MaxTemp.reg, "MaxTemp.coeffs" = MaxTemp.coeffs, "TimeAtMax.reg" = TimeAtMax.reg, "TimeAtMax.coeffs" = TimeAtMax.coeffs, 
                     ...
                     "sensor.depths" = sensor.depths, "Shallowest" = Shallowest, "FullTime" = FullTime, "StartTime" = StartTime, "EndTime" = EndTime)
  return(model.list)
}
