...
#' 
#' chamaecrista.seeds <- survival_depth(survival.value = 0.8, deltaS = 112, deltaH = 147741, inc = 0.1, model = model_example) #default temporal resolution, increment, and temp. Chamaecrista (cassia) seed survival, for parameters see Martin et al (1969)
...
survival.depth <- function(survival.value, deltaS, deltaH, temp = 48, inc = 0.5, res = 1, model){
  #res checked in temp_over_time function
  if(mode(survival.value)!= "numeric" | length(survival.value) != 1 | survival.value > 1 | survival.value < 0){  #is survivalValue valid
