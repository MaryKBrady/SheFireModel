...
#' @author MaryKBrady
#' @seealso \code{\link{shefire}}, \code{\link{set_temp_intervals}}
...
#' 
#' binned5cm <- temp_intervals(x = 5, bin = 5, model = model_example) #res defaults to 1 minute
#' binned5cm.res <- temp_intervals(x = 5, bin = 5, res = 0.5, model = model_example) #changed temporal resolution to .5 min
#' @export

temp_intervals <- function(x, bin = 5, res = 1, model){
